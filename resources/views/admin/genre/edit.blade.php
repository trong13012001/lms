<x-admin-layout>
    ...
    <x-slot name="breadcrumb">
        {{ Breadcrumbs::render('admin.genre.edit', $genre) }}
    </x-slot>
    <form method="POST" action="{{ route('admin.genre.update',$genre) }}" enctype="multipart/form-data" id="book-form" >
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-7">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <textarea rows="3" class="mb-3 d-none" name="description" id="description" placeholder="Nhập giới thiệu thể loại ">{{ old('description',$genre['description']) }}</textarea>
                                </div>
                            </div>

                        </div>
                        @if (auth()->user()->can('admin.genre.update'))
                        ...
                        <div class="body-card">
                            <div class="form-group mb-3">
                                <upload-file title="Ảnh đại diện" name="image" id="image" type="image"
                                    url="{{ route('unisharp.lfm.show') }}" value="{{ old('image',$genre['image']) }}" />
                            </div>
                        </div>
                    </div>
                </div>
