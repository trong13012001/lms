@props(['id', 'customers', 'value'])

<div class="modal fade modal-del" id="modal-issued-book-edit-{{ $id }}" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-gd-sea-op">
                <h4 class="modal-title text-white">Mượn sách</h4>
            </div>
            <form id="tag-form" action="{{ route('admin.issued_book.update', $id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="modal-body pb-0">
                    <div class="mb-3">
                        ...
                        <select class="choices form-select" name="customer_id" id="customer_id" required>
                            ...
                            @foreach ($customers as $customer)
                                <option value="{{ $customer['id'] }}" {{ $customer->id == old('customer_id', $value['customer_id']) ? 'selected' : '' }} >
                                    {{ $customer['name'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="js-flatpickr form-control" id="return_date" name="return_date"
                               placeholder="Y-m-d" value="{{ old('return_date',$value->return_date) }}">
                    </div>
                </div>
